...
blockImage.src = 'path/to/blocos.png';
destructibleBlockImage.src = 'path/to/blocos3.png';
enemyImage.src = 'path/to/chefao1.webp';
bombImage.src = 'path/to/bomba.webp';
explosionImage.src = 'path/to/explosao.webp';

let playerPosition = { x: 0, y: 0 };
let blocks = [
    { x: 2, y: 0, type: 'indestructible' },
    { x: 2, y: 1, type: 'destructible', points: 10 },
    { x: 3, y: 2, type: 'destructible', points: 20 },
    { x: 6, y: 3, type: 'indestructible' },
    { x: 5, y: 4, type: 'destructible', points: 30 },
    { x: 6, y: 5, type: 'indestructible' },
    { x: 4, y: 7, type: 'indestructible' },
    { x: 3, y: 8, type: 'destructible', points: 40 },
    // outros blocos
];
let enemies = [
    { x: 5, y: 5 },
...
function explodeBomb(x, y) {
    ...
    explosionArea.forEach(pos => {
        if (pos.x >= 0 && pos.x < gridSize && pos.y >= 0 && pos.y < gridSize) {
            ...
            // Checa se o jogador está na área de explosão
    ...
    setTimeout(() => {
        ...
        if (enemies.length === 0) {
            alert("Você venceu! Parabéns por chegar até aqui!");
            window.location.href = 'winner.html';  // Mude para o nome do arquivo da segunda fase
        } else {
